...
class EvalDatasetLoader:
    ...
    def load_dataset(
        ...
        fallback_question_keys: List[str] = ["input", "premise", "context"],
        fallback_answer_keys: List[str] = ["output", "hypothesis", "label", "mc1_targets", "mc2_targets"],
        ...
        """
        cache_key = self._generate_cache_key(dataset_name, subset, split)
        ...
        try:
            ...
            if cache_path.exists():
                ...
                logger.warning("Cache data invalid or empty, reloading from source")
            ...
            if not dataset or len(dataset) == 0:
                raise ValueError(f"Dataset {dataset_name} is empty or failed to load")
            
            # Log dataset structure for debugging
            sample_item = dataset[0]
            logger.debug(f"Dataset structure: {list(sample_item.keys())}")
            ...
            for item in dataset:
                try:
                    ...
                    question = None
                    for key in [question_key] + fallback_question_keys:
                        if key in item:
                            question = item[key]
                            break
                    ...
                    if isinstance(answer, list):
                        ...
                        for possible_key in ['text', 'answer', 'value', 'label']:
    ...
    def _generate_cache_key(self, dataset_name: str, subset: Optional[str], split: str) -> str:
    ...
    def _save_to_cache(
        ...
        if not questions or not answers:
            logger.warning("Attempting to cache empty dataset, skipping")
            return
        ...
        logger.info(f"Cached {len(questions)} examples to {cache_path}")
    ...
    def _load_from_cache(
        ...
        try:
            ...
            logger.info(f"Loaded {len(questions)} examples from cache")
            return questions, answers
            
        except Exception as e:
            logger.error(f"Error loading cache {cache_path}: {str(e)}")
            return [], []
